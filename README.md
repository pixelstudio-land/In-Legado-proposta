# In-Legado-proposta

Landing page de proposta comercial exclusiva para Leonardo — Plataforma de Formulários Conversacionais desenvolvida pela Pixel Studio.

## Contato Configurado

- **WhatsApp:** `+55 (11) 91339-3797`
- **E-mail:** `contato@pixelstudio.com`

## GitHub Pages

Este repositório está configurado para deploy via GitHub Pages.  
Acesse: `https://pixelstudio-land.github.io/In-Legado-proposta/`

## Estrutura

```
index.html    ← LP completa (single-file, zero dependências externas)
README.md     ← Este arquivo
```
